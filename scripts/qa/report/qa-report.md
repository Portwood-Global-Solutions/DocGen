...
**Org** `docgen-verify` · **Run** 2026-07-26T00:49:10.223Z · **Duration** 333s
...
| ----------- | ----------- | -----: | -----: | ------: | ----: |
| `ui-runner` | End-user UI |     75 |      4 |       1 | 94.9% |
...
- ✅ the generated document's title resolves merge tokens against the record — Document_Title_Format\_\_c = "UIQA-ms12yhvc {Name}" → title was "UIQA-ms12yhvc UIQA Alpha", expected "UIQA-ms12yhvc UIQA Alpha"
...
- ✅ pressing Create Document in Save to Record mode puts the document ON the record — ContentDocument "UIQA-ms12yhvc UIQA Alpha" linked to 001O5000040r99fIAA
...
- ✅ pressing Create Document in Download mode downloads the document to the browser — the browser received "UIQA-ms12yhvc UIQA Alpha.pdf"
...
- ✅ sending a signature request tells the user it worked — component text: Signature Links Generated! Provide each signer with their unique secure link: UIQA-ms12yhvc Signer Signer https://<CONFIGURE_SITE_URL_IN_SETUP>/apex/portwoodglobal\_\_DocGenSignaturePdf?
- ✅ sending writes a DocGen_Signature_Request\_\_c tied to this record and template — request a08O5000013zGQbIAM: record=001O5000040r99fIAA (expected 001O5000040r99fIAA), status=Sent, order=Parallel, sourceDoc=068O500000MIDuzIAH
- ✅ sending writes exactly one DocGen_Signer\_\_c carrying what was typed — 1 signer row(s) (expected 1); first = name "UIQA-ms12yhvc Signer" (typed "UIQA-ms12yhvc Signer"), email "uiqa-ms12yhvc@example.com" (typed "uiqa-ms12yhvc@example.com"), role "Signer" (typed "Signer"),
...
- ✅ pressing Run creates a bulk job on the server — DocGen_Job\_\_c a03O500003h7WdZIAU status Completed
...
- ✅ the failing job records WHY each record failed — Error_Log\_\_c = 001O5000040r99fIAA — portwoodglobal.DocGenException: Error retrieving template data: No template file found (active or attached). 001O5000040r99gIAA — portwoodglobal.DocGenException: Er
- ✅ the Recent Jobs list shows the error count to the user — Recent Jobs did not show "UIQA-ms12yhvc-err" with "2 errors" — a user would see the run as finished with no indication anything went wrong
