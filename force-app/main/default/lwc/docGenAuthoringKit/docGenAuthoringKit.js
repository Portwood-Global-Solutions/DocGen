...
// ---------------------------------------------------------------------------
// Canvas starters — built as BOXES, not converted from HTML
// ---------------------------------------------------------------------------

/**
 * Canvas-native starter bodies.
 *
 * These emit the canvas contract directly — pinned and flow boxes with real
 * coordinates — rather than being run through the HTML importer. The importer is
 * lossless on content, but it can only ever produce what it is given: a page of prose
 * arrives as one text box, and an authored table stays markup because reshaping an
 * arbitrary table into the table model was measured to drop merge tags. That is the
 * right behaviour for someone else's document and the wrong starting point for a
 * starter, where the whole point is a laid-out set of pieces you can grab and move.
 *
 * Written as boxes, a starter opens with a title you can drag, a details table the
 * table tool can edit, and a line-item loop already bound to a relationship.
 *
 * Geometry is in inches against the CONTENT area (page minus margins), matching
 * canvasModel: pinned boxes carry x/y, flow boxes carry the y they would like to sit
 * at and stack in document order.
 */
const CANVAS_PAGE = { w: 8.5, h: 11, margin: 0.75 };
...
/** A pinned box: lands exactly here, does not move when content above it grows. */
function cvPin(x, y, w, h, inner, css) {
    return (
        '<div class="dg-pin" data-dg-x="' +
        x +
        '" data-dg-y="' +
        y +
        '" data-dg-w="' +
        ...
        '" data-dg-h="' +
        h +
        '" data-dg-mode="pinned" data-dg-z="0" style="left: ' +
        x +
        'in; top: ' +
        y +
        'in; width: ' +
...
/** A flow box: sits in normal flow so its content can grow and paginate. */
function cvFlow(y, w, h, inner, css, gap) {
    return (
        '<div class="dg-flow" data-dg-x="0" data-dg-y="' +
        y +
        '" data-dg-w="' +
        w +
        '" data-dg-h="' +
        h +
        '" data-dg-mode="flow" data-dg-z="0" style="margin: ' +
        (gap == null ? 0 : gap) +
        'in 0 0 0in; width: ' +
        w +
        'in; padding: 0pt;' +
...
/**
 * A table box in the shape the table tool reads back: header in <thead>, the repeating
...
function cvDocument(boxes, opts) {
    const o = opts || {};
    const margin = o.margin == null ? CANVAS_PAGE.margin : o.margin;
    const landscape = o.orientation === 'Landscape';
    const paperW = landscape ? CANVAS_PAGE.h : CANVAS_PAGE.w;
    const paperH = landscape ? CANVAS_PAGE.w : CANVAS_PAGE.h;
    const w = round3(paperW - margin * 2);
    const h = round3(paperH - margin * 2);
    const css =
        '@page { size: Letter ' +
    ...
    return {
        html:
            '<!DOCTYPE html>\n<html>\n<head>\n<meta charset="utf-8" />\n<style>\n' +
            css +
            '</style>\n</head>\n<body>\n<div class="dg-artboard" data-dg-artboard="1" style="min-height: ' +
            h +
            'in;">\n  ' +
            boxes.join('\n  ') +
            '\n</div>\n</body>\n</html>',
...
/** The first child relationship with fields, which every list-shaped starter needs. */
function firstChild(shape) {
    return ((shape && shape.children) || []).find((c) => (c.fields || []).length) || null;
}

function detailFields(shape, max) {
    return [...((shape && shape.baseFields) || []), ...((shape && shape.parentFields) || [])]
        .filter((f) => f && f.toLowerCase() !== 'id')
        .slice(0, max || 6);
}

function buildCanvasReport(shape) {
    const d = cvDocument([], {});
    const W = d.width;
    const fields = detailFields(shape, 6);
    const child = firstChild(shape);
    const boxes = [];
    boxes.push(
        cvPin(0, 0, W, 0.45, '<p><b>{Name}</b></p>', ' font-family: sans-serif; font-size: 20pt; color: #1f3a5f;')
    );
    boxes.push(
        cvPin(
            0,
    ...
    if (child) {
        ...
        boxes.push(
            cvFlow(
                ...
                W,
                0.3,
                '<p><b>' + cvEsc(humanizeField(child.relationshipName || child.alias)) + '</b></p>',
                ' font-family: sans-serif; font-size: 13pt; color: #1f3a5f;',
                0.35
            )
        );
        boxes.push(
            cvFlow(
                ...
                1.5,
                cvTable(
                    cf.map((f) => ({ label: humanizeField(f), tag: '{' + f + '}' })),
                    child.relationshipName || child.alias
                ),
                '',
                0.1
            )
    ...
    return cvDocument(boxes, {}).html;
}

function buildCanvasInvoice(shape) {
    const d = cvDocument([], {});
    const W = d.width;
    const child = firstChild(shape);
    const cf = child ? (child.fields || []).slice(0, 3) : [];
    const amount = cf.find((f) => /amount|price|total/i.test(f)) || cf[cf.length - 1];
    const boxes = [];
    boxes.push(cvPin(0, 0, 3.5, 0.4, '<p><b>INVOICE</b></p>', ' font-family: sans-serif; font-size: 22pt;'));
    boxes.push(
        cvPin(
            W - 3,
            0,
            3,
            0.9,
            '<p>{Name}<br />{Today:MMMM d, yyyy}</p>',
            ' font-family: sans-serif; font-size: 11pt; text-align: right;'
        )
    );
    boxes.push(
        cvPin(0, 0.6, 3.5, 0.9, '<p><b>Billed to</b><br />{Name}</p>', ' font-family: sans-serif; font-size: 11pt;')
    );
    if (child) {
        boxes.push(
            cvFlow(
                ...
                cvTable(
                    ...
                    amount
                        ? cf.map((f, i) =>
                              i === cf.length - 1
                                  ? '{SUM:' + (child.relationshipName || child.alias) + '.' + amount + ':currency}'
                                  : i === 0
                                    ? 'Total'
                                    : ''
                          )
...
function buildCanvasLetter(shape) {
    const d = cvDocument([], {});
    const W = d.width;
    const boxes = [];
    boxes.push(
        cvPin(0, 0, W, 0.4, '<p><b>{Name}</b></p>', ' font-family: sans-serif; font-size: 18pt; color: #1f3a5f;')
    );
    boxes.push(cvPin(0, 0.6, 3, 0.25, '<p>{Today:MMMM d, yyyy}</p>', ' font-family: sans-serif; font-size: 11pt;'));
    boxes.push(
        cvFlow(
            1.2,
            W,
            3,
            '<p>Dear {Name},</p><p>Replace this paragraph with what you want to say. Every field in your Query ' +
                'Config is available as a merge tag, and the text reflows as the document grows.</p>' +
                '<p>Sincerely,<br />{RunningUser.Name}</p>',
...
function buildCanvasAgreement(shape) {
    const d = cvDocument([], {});
    const W = d.width;
    const boxes = [];
    boxes.push(
        cvPin(0, 0, W, 0.4, '<p><b>Agreement</b></p>', ' font-family: sans-serif; font-size: 18pt; text-align: center;')
    );
    boxes.push(
        cvFlow(
            0.7,
            W,
            2.5,
            '<p>This agreement is entered into by {Name} on {Today:MMMM d, yyyy}.</p>' +
                '<ol><li>Replace this term with your own.</li><li>Add as many as you need — the list flows.</li></ol>',
            ' font-family: sans-serif; font-size: 11pt;',
            0.7
        )
    );
    boxes.push(
        cvPin(0, 8.2, 2.6, 0.6, '{@Signature_Customer:1:Full}', '', '').replace(
            'data-dg-mode="pinned" data-dg-z="0"',
            'data-dg-mode="pinned" data-dg-z="0" data-dg-sig-role="Customer" data-dg-sig-order="1" data-dg-sig-type="Full" data-dg-sig-inline="0"'
        )
    );
    boxes.push(
        cvPin(0, 8.9, 1.6, 0.4, '{@Signature_Customer:1:Date}', '').replace(
            'data-dg-mode="pinned" data-dg-z="0"',
            'data-dg-mode="pinned" data-dg-z="0" data-dg-sig-role="Customer" data-dg-sig-order="1" data-dg-sig-type="Date" data-dg-sig-inline="0"'
        )
    );
    boxes.push(
        cvPin(W - 2.6, 8.2, 2.6, 0.6, '{@Signature_Company:2:Full}', '').replace(
            'data-dg-mode="pinned" data-dg-z="0"',
            'data-dg-mode="pinned" data-dg-z="0" data-dg-sig-role="Company" data-dg-sig-order="2" data-dg-sig-type="Full" data-dg-sig-inline="0"'
        )
    );
    return cvDocument(boxes, {}).html;
}

function buildCanvasCertificate(shape) {
    const d = cvDocument([], { orientation: 'Landscape', margin: 0.5 });
    const W = d.width;
    const boxes = [];
    boxes.push(
        cvPin(
            ...
            W,
            d.height,
            '<div data-dg-shape="rect" style="width: 100%; height: ' +
                round3(d.height - 0.1) +
                'in; border: 3pt solid #1f3a5f; font-size: 0; line-height: 0;"></div>'
        )
    );
    boxes.push(cvPin(0.8, 1.2, W - 1.6, 0.4, '<p>This certifies that</p>', ' font-size: 13pt; text-align: center;'));
    boxes.push(
        cvPin(
            0.8,
            1.8,
            ...
            0.8,
            '<p><b>{Name}</b></p>',
            ' font-family: serif; font-size: 30pt; color: #1f3a5f; text-align: center;'
        )
    );
    boxes.push(
        cvPin(
            ...
            W - 1.6,
            0.5,
            '<p>has completed the requirements — replace this line.</p>',
            ' font-size: 12pt; text-align: center;'
        )
    );
    boxes.push(cvPin(0.8, 4.6, W - 1.6, 0.3, '<p>{Today:MMMM d, yyyy}</p>', ' font-size: 11pt; text-align: center;'));
    return cvDocument(boxes, { orientation: 'Landscape', margin: 0.5 }).html;
}

const CANVAS_BUILDERS = {
    report: buildCanvasReport,
    invoice: buildCanvasInvoice,
    letter: buildCanvasLetter,
    agreement: buildCanvasAgreement,
    certificate: buildCanvasCertificate
};

/**
 * A starter as a CANVAS document — boxes already placed, ready to drag.
 *
 * Falls back to the report layout for an unknown key, the same way buildStarterHtml
 * does, so a new starter key can never produce an empty template.
 */
export function buildCanvasStarter(starterKey, shape) {
    const builder = CANVAS_BUILDERS[starterKey] || buildCanvasReport;
    return builder(shape || {});
}
