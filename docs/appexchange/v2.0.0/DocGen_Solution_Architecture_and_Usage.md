...
**Prior listing version (AppExchange):** v1.56.0 (`04tal000006i1rNAAQ`) — this submission addresses all 30 findings from the AppExchange security review against the v1.56 listing. v2.0 also rolls forward ~44 versions of feature work since v1.56 (V3 query trees, chart engine, signature v3 with PIN second factor + multi-signer + guided placements, HTML templates, giant-query batching).
...
DocGenDataRetriever fetches record data via SOQL
   ...
   - Package-internal config queries (DocGen_Template__c,
     ...
     an explicit Schema.sObjectType.<Object>.isAccessible() check
     at the @AuraEnabled entry point, then run WITH SYSTEM_MODE.
     SYSTEM_MODE is required because USER_MODE strict-FLS strips
     namespaced custom fields when subscriber admin profiles
...
| Check                                             | Status                                                                                                                               |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| Salesforce Code Analyzer (Security + AppExchange) | 0 High severity violations (38 Moderate false positives — pre-existing pmd:AvoidLwcBubblesComposedTrue and pmd:ProtectSensitiveData) |
| External callouts                                 | None                                                                                                                                 |
| Session ID usage                                  | None                                                                                                                                 |
...
| Apex tests                                        | 1,436 local tests, 76% org-wide coverage                                                                                             |
...
- `DocGen_False_Positive_Report.md` (this folder) — v2.0 hybrid-pattern rationalizations for the Checkmarx false-positive disposition.
