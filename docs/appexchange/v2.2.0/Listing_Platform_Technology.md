...
**Apex patterns and class counts (v2.1.0)**
...
- **Test classes (29 classes)** — 1,449 test methods, org-wide code coverage 76%, enforced pre-release. Every release run also passes 11 end-to-end anonymous Apex scripts (`scripts/e2e-01` through `e2e-08` + four `e2e-07-syntax*` syntax suites).

**Sharing and security (v2.1.0)**
...
- Every admin `@AuraEnabled` / `@InvocableMethod` entry point opens with an explicit `Schema.sObjectType.<Object>.isAccessible | isCreateable | isUpdateable()` gate (shipped in v2.0). This is the documented enforcement signal the `sfge:ApexFlsViolation` rule pattern-matches on, and it is the reviewer's stated first alternative ("enforce CRUD checks on the object… **or** alternatively use USER_MODE") from the v1.56 finding-resolution language.
- v2.1.0 adds **per-field `Schema.SObjectField.getDescribe().is{Createable,Updateable,Accessible}()` checks via the new `DocGenFlsGuard` helper** immediately before every DML and `WITH SYSTEM_MODE` SOQL statement. 243 call sites across 19 controllers (70 DML guard sites across 9 controllers + 173 SOQL guard sites across 18 classes). This implements the AND half of the reviewer's stated finding-resolution language ("enforce CRUD checks on the object AND FLS checks on the fields before performing any DML operation") line-for-line.
- Standard-object SOQL (`ContentVersion`, `ContentDocumentLink`, `User`, `OrgWideEmailAddress`, etc.) runs `WITH USER_MODE` throughout. Package-namespaced custom-object SOQL/DML runs `WITH SYSTEM_MODE` behind both the Schema-CRUD gate AND the DocGenFlsGuard per-field check, with `/* code-analyzer-suppress ApexFlsViolation */` and inline justification — required because USER_MODE strict-FLS strips package-namespaced custom fields when subscriber admin profiles haven't been granted FLS individually per release (reproducible in package-build orgs).
- Guest signing endpoints route through `DocGenSignatureGuestSecurity` (shipped in v2.0) for Schema-CRUD describe checks + 64-char SHA-256 hex token-shape validation + per-operation field allowlists, all documented inline at every call site. v2.1.0 layers DocGenFlsGuard per-field guards on top.
...
- **Class counts:** 27 → 42 non-test, 19 → 29 test (846 → 1,449 tests at 75% → 76% coverage).
- **LWC bundles:** 17 → 18.
...
- **SOQL/DML execution mode:** v2.0 shipped the hybrid Schema-CRUD-gate + SYSTEM_MODE pattern across every admin endpoint, replacing the v1.42/v1.99 "permission sets are the CRUD/FLS boundary" rationalization that the AppExchange reviewer rejected against the v1.56 listing.
- **v2.1.0 per-field FLS guard layer:** new `DocGenFlsGuard` helper class performs explicit `Schema.SObjectField.getDescribe().is{Createable,Updateable,Accessible}()` checks at every admin DML and `WITH SYSTEM_MODE` SOQL site. 243 guard call sites across 19 controllers (70 DML + 173 SOQL). Closes the 562 Checkmarx findings (FLS Create 118 + FLS Update 104 + USER_MODE Missing 340) returned on the v2.0 source scan.
- **Guest signing:** `DocGenSignatureGuestSecurity` helper class (shipped in v2.0) centralizes Schema-CRUD describe checks + token-shape validation + per-operation field allowlists at every guest entry point. v2.1.0 layers DocGenFlsGuard per-field guards on top.
...
_File maintained at `docs/appexchange/v2.1.0/Listing_Platform_Technology.md` so future re-submissions can pull this verbatim or update in place._
