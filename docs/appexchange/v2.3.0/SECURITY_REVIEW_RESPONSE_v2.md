# Security Review Response — Portwood DocGen v2.3.0
...
**This submission version:** v2.3.0 (`04tVx000000ZxDJIA0`, build `2.3.0-1`), promoted 2026-05-23
**Prior submissions:** v2.1.0 (`04tVx000000Zw5xIAC`, promoted 2026-05-22), v2.2.0 (`04tVx000000ZxBhIAK`, promoted 2026-05-23)
**Previous report:** "Security Report for Portwood DocGen Managed- app record for SR"
**Response date:** 2026-05-23

This document responds to each finding in the prior AppExchange security review (against the v1.56 listing) and points to the specific commits/files in v2.0 (clickjacking + object-level CRUD gate), v2.1.0 (per-field FLS guard), v2.2.0 (guest-aware variant of the per-field FLS guard — write paths), and v2.3.0 (guest-aware FLS read paths — the completion patch) that address it. v2.0 / v2.1.0 / v2.2.0 / v2.3.0 also roll forward ~45 versions of feature work since v1.56 (V3 query trees, chart engine, signature v3 with PIN second factor + multi-signer + guided placements, HTML templates, giant-query batching). Where SYSTEM_MODE is retained, this document explains the structural reason it cannot be replaced with USER_MODE without creating the very vulnerability the finding asks us to prevent.

> **v2.3.0 update (one-paragraph summary for the reviewer):** v2.3.0 completes the v2.2.0 hotfix. v2.2.0 added the guest-aware `guestAssertCreateable / guestAssertUpdateable / guestAssertAccessible` helpers and swapped the 18 admin write-guards in `DocGenSignatureController.cls` to the guest variants — but left the 36 read-guards (`DocGenFlsGuard.assertAccessible`) as admin variants. Those throw the same way on guest context, just with the per-field FLS describe verdict on the SOQL select-list. The `DocGen_Guest_Signature` permset DOES grant `<readable>true</readable>` on every field in the read allowlist, but `Schema.SObjectField.getDescribe().isAccessible()` returns FALSE for guest profiles even when the permset grants it — the same platform inconsistency that drove the v2.1 → v2.2 write-path fix. v2.3.0 swaps the 36 read-guard sites (34 in `DocGenSignatureController.cls`, 2 in `DocGenAuthenticatorController.cls`'s public verifier endpoints) to the existing `guestAssertAccessible` helper shipped in v2.2. No new classes, no new methods, no new tests, no metadata changes. The per-field `Schema.SObjectField.getDescribe().isAccessible()` probe still fires at every site for the Checkmarx CxSAST pattern-match signal; only the verdict is bypassed when `UserInfo.getUserType() == 'Guest'`.
