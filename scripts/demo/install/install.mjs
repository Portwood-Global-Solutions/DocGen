...
// A second manifest can drive the same installer against the same body library.
// The SDocs-replacement demo (scripts/demo/sdocs/) ships its own so it can be
// installed on its own, without dragging in the 31-template catalog — and
// without forking this file. Relative paths resolve from the repo root.
const manifestArg = process.argv.find((a) => a.startsWith('--manifest='));
const MANIFEST = manifestArg
    ? path.resolve(REPO_ROOT, manifestArg.split('=').slice(1).join('='))
...
async function orgInfo(alias) {
    ...
    // sf 2.9x REDACTS accessToken in `org display` — the field comes back as the
    // literal "[REDACTED] Use 'sf org auth show-access-token' to view", which then
    // goes out as the bearer token and every upload fails with a 401
    // INVALID_AUTH_HEADER that reads like an expired org rather than a CLI change.
    // The dedicated command is now the only way to get it.
    if (!r.accessToken || /REDACTED|show-access-token/.test(r.accessToken)) {
        const { stdout: tok } = await run('sf', ['org', 'auth', 'show-access-token', '--target-org', alias, '--json'], {
            maxBuffer: 1e8
        });
        r.accessToken = JSON.parse(tok).result.accessToken;
    }
...
async function uploadCv(org, filePath, title) {
    ...
    const { stdout } = await run(
        ...
        [
            ...
            '-F',
            `entity_content=${meta};type=application/json`,
            '-F',
            `VersionData=@${filePath};type=${MIME[ext] || 'application/octet-stream'}`
        ],
        { maxBuffer: 1e8 }
    );
...
/**
 * Asks the ORG which prefix its DocGen_Template__c actually carries.
 *
 * `sf org display` reports the org's OWN namespace, which is the wrong question.
 * A subscriber org with the managed package INSTALLED has no namespace of its own,
 * so that field comes back empty and the installer generated bare
 * `DocGen_Template__c` references — which compile-fail with "Invalid type" against
 * an org where the object is really `portwoodglobal__DocGen_Template__c`.
 *
 * The two cases are genuinely different orgs, and neither is inferable from org
 * metadata: a --no-namespace scratch org (source-deployed, bare names) and a real
 * install (namespaced). So probe for the object instead of predicting it.
 */
async function detectNamespace(org) {
    for (const ns of ['portwoodglobal__', '']) {
        const { stdout } = await run('curl', [
            ...
            '-w',
            '%{http_code}',
            `${org.url}/services/data/v60.0/sobjects/${ns}DocGen_Template__c/describe`,
            '-H',
            `Authorization: Bearer ${org.token}`
        ]);
        if (stdout.trim() === '200') {
...
async function main() {
    ...
    if (!ORG) {
        console.error(
            'usage: node scripts/demo/install/install.mjs <orgAlias> [--only=key1,key2] [--manifest=path/to/manifest.json]'
        );
        process.exit(1);
    }
    ...
    let ns;
    if (process.env.DOCGEN_DEMO_NS !== undefined) {
        ns = process.env.DOCGEN_DEMO_NS;
    } else {
        ns = await detectNamespace(org);
        if (ns === null) {
            console.error(
                `\nERROR: ${ORG} has no DocGen_Template__c under either "portwoodglobal__" or the ` +
                    `default namespace. Portwood is not installed (or not deployed) in this org. ` +
                    `Install the package or deploy force-app first, then re-run.`
            );
            process.exit(1);
        }
    }
    ...
    for (const t of templates) {
        ...
        specs.push({
            ...
            name: t.name,
            type: t.type,
            output: t.output || (t.type === 'HTML' ? 'PDF' : 'Native'),
            baseObject: t.baseObject,
            category: t.category || 'DocGen Demo',
            titleFormat: t.titleFormat || t.name + ' - {Name}',
            description: t.description || '',
            pageSize: t.pageSize || 'Letter',
            orientation: t.orientation || 'Portrait',
            headerHtml: t.headerHtml || '',
            footerHtml: t.footerHtml || '',
            customMargins: t.customMargins || '',
    ...
    for (let i = 0; i < specs.length; i += BATCH) {
        ...
        stdout
            .split('\n')
            .filter((l) => /USER_DEBUG/.test(l) && /DEMO-INSTALL (CREATED|ERROR)/.test(l))
            .forEach((l) => {
...
for (Object o : specs) {
  try {
    ...
    // Canvas is HTML-backed — DocGenService.isHtmlBacked treats the two the same,
    // and the Canvas designer reads the body through the SAME
    // 'docgen_html_body_<templateId>' lookup. Without Canvas here, an installed
    // canvas template generates a correct PDF and then opens onto a blank artboard,
    // which is the exact failure this block was written to fix for HTML.
    String bodyType = (String) s.get('type');
    if (bodyType == 'HTML' || bodyType == 'Canvas') {
