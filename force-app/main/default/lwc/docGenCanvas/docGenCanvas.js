...
import { extractQueryShape, buildAiPrompt } from 'c/docGenAuthoringKit';
import isAiAvailable from '@salesforce/apex/DocGenAiTemplateController.isAiAvailable';
import generateCanvasPreview from '@salesforce/apex/DocGenAiTemplateController.generateCanvasPreview';
import generateFragment from '@salesforce/apex/DocGenAiTemplateController.generateFragment';
...
export default class DocGenCanvas extends LightningElement {
    ...
    // ---- Agentforce ------------------------------------------------------
    //
    // Two jobs, and they are deliberately not the same job.
    //
    // DRAFT builds a whole document and imports it. htmlToCanvas is structural, not
    // geometric, so what lands is a stack of flow blocks in document order — content to
    // arrange, not a finished layout. Saying so in the UI matters: "generate my canvas
    // layout" is what people will assume this does.
    //
    // REWRITE changes the selected block only. The alternative — round-tripping the
    // serialized canvas through the model — means trusting it to return every
    // data-dg-x/y/w/h/id/anchor byte-for-byte, and an anchor id it rewrote would
    // silently re-point an element link at a different box. Here geometry and links
    // never leave the client.
    //
    // Neither path saves. generateTemplateBody writes a ContentVersion of its own,
    // which would leave the template record and this in-memory canvas describing two
    // different documents — the divergence the canvas exists to avoid.
    @track aiAvailable = false;
    @track aiPanelOpen = false;
    @track aiBusy = false;
    @track aiMode = 'rewrite';
    @track aiInstruction = '';
    @track aiSummary = '';
    @track aiFindings = [];
    @track aiError = null;
    @track aiConfirmDiscard = false;
    ...
    get showAiButton() {
        return this.aiAvailable && !!this.templateId;
    }

    get aiModeOptions() {
        return [
            { label: 'Rewrite the selected element', value: 'rewrite' },
            { label: 'Draft a new document', value: 'draft' }
        ];
    }

    get isAiRewrite() {
        return this.aiMode === 'rewrite';
    }

    /** Only a box whose content is markup can be rewritten as markup. */
    get aiCanRewrite() {
        const b = this.selectedBox;
        return !!b && (!b.kind || b.kind === 'text');
    }

    get aiRewriteBlocked() {
        if (!this.isAiRewrite || this.aiCanRewrite) {
            return null;
        }
        const b = this.selectedBox;
        if (!b) {
            return 'Select a text element on the page first, or switch to "Draft a new document".';
        }
        return (
            'A ' +
            b.kind +
            ' element is built from its own settings rather than from markup, so there is nothing here to rewrite. ' +
            'Select a text element instead.'
        );
    }

    get aiInstructionLabel() {
        return this.isAiRewrite ? 'What should I change?' : 'What should this document be?';
    }

    get aiInstructionPlaceholder() {
        return this.isAiRewrite
            ? 'e.g. Make this a two-column summary, bold the totals, and add the close date on the right.'
            : 'e.g. A one-page account summary: a title, a details panel, a table of open opportunities with a total row, and a footer.';
    }

    get aiSubmitLabel() {
        if (this.aiBusy) {
            return this.isAiRewrite ? 'Rewriting…' : 'Drafting…';
        }
        return this.isAiRewrite ? 'Rewrite element' : 'Draft document';
    }

    /** Drafting replaces the whole board, so it asks first when there is work to lose. */
    get aiNeedsDiscardConfirm() {
        return !this.isAiRewrite && this.boxCount > 0 && !this.aiConfirmDiscard;
    }

    get aiSubmitDisabled() {
        return this.aiBusy || (this.isAiRewrite && !this.aiCanRewrite) || this.aiNeedsDiscardConfirm;
    }

    get hasAiReport() {
        return this.aiFindings && this.aiFindings.length > 0;
    }
    ...
    handleAiOpen() {
        ...
        this.aiFindings = [];
        this.aiError = null;
        this.aiConfirmDiscard = false;
        // Rewriting is the safe default whenever there is something selected to
        // rewrite — drafting throws the board away.
        this.aiMode = this.aiCanRewrite ? 'rewrite' : 'draft';
    ...
    handleAiInstructionChange(event) {
        this.aiInstruction = event.target.value;
    }

    handleAiConfirmDiscardChange(event) {
        this.aiConfirmDiscard = !!event.target.checked;
    }

    /**
     * The instruction as TYPED, not as last committed.
     *
     * lightning-textarea fires `change` on blur, so clicking straight from the box to
     * the submit button submits the previous value — or, on a first use, an empty one.
     * docGenAdmin hit exactly this and sent Agentforce an edit instruction with nothing
     * in it.
     */
    get _aiInstructionNow() {
        const el = this.template.querySelector('.dg-ai-instruction');
        const live = el && el.value != null ? el.value : this.aiInstruction;
        return (live || '').trim();
    }
    ...
    // 8.5 rather than 8.500000000000002 — the number goes into a prompt, and a page
    // width with a floating-point tail reads as noise the model has to interpret.
    _round2(n) {
        return Math.round(n * 100) / 100;
    }

    _mapFindings(findings) {
        return (findings || []).map((f, i) => ({
            ...
            badgeClass:
                f.action === 'warning'
                    ? 'dg-ai-badge dg-ai-badge_warn'
                    : f.action === 'repaired'
                      ? 'dg-ai-badge dg-ai-badge_ok'
                      : 'dg-ai-badge'
        }));
    }

    async handleAiSubmit() {
        const instruction = this._aiInstructionNow;
        if (!instruction) {
            this.aiError = this.isAiRewrite
                ? 'Describe the change you want first.'
                : 'Describe the document you want first.';
            return;
        }
        ...
        try {
            if (this.isAiRewrite) {
                await this._aiRewriteSelected(instruction);
            } else {
                await this._aiDraftDocument(instruction);
            }
        } catch (e) {
            this.aiError = this.errText(e);
        } finally {
            this.aiBusy = false;
        }
    }

    async _aiRewriteSelected(instruction) {
        const box = this.selectedBox;
        if (!box) {
            this.aiError = 'Select an element on the page first.';
            return;
        }
        const current = box.html != null && box.html !== '' ? box.html : box.text || '';
        const prompt = buildAiPrompt(this._aiShape(), {
            dataSourceMode: this.baseObject === 'FlowJsonData' ? 'flow' : 'record',
            fragment: true,
            currentBody: current,
            docDescription: instruction
        });
        const res = await generateFragment({ prompt, previousFragment: current });
        const html = res.html || '';
        if (!html.trim()) {
            this.aiError = 'Agentforce returned nothing. Try again, or make the instruction more specific.';
            return;
        }
        // Through the same sanitizer every other authored fragment goes through, so a
        // model cannot introduce markup the rest of the editor would not accept —
        // after reducing a whole document to its body, because models return one
        // sometimes however plainly the prompt asks for a fragment.
        const clean = sanitizeInline(toFragment(html));
        this.applyToBox(box.id, { html: clean, text: '' });
        this.aiSummary = res.summary || 'Element rewritten.';
        this.aiFindings = this._mapFindings(res.findings);
        this.statusText = 'Element rewritten — undo restores it';
    }

    async _aiDraftDocument(instruction) {
        // The REAL artboard, so the coordinates it writes fit the page the author is on
        // rather than an assumed Letter portrait.
        const geo = this.geo;
        const prompt = buildAiPrompt(this._aiShape(), {
            dataSourceMode: this.baseObject === 'FlowJsonData' ? 'flow' : 'record',
            canvas: true,
            pageWidthIn: this._round2(geo.w),
            pageHeightIn: this._round2(geo.h),
            docDescription: instruction
        });
        const res = await generateCanvasPreview({ prompt });
        const html = res.html || '';
        if (!html.trim()) {
            this.aiError = 'Agentforce returned nothing. Try again, or make the instruction more specific.';
            return;
        }
        this.pushHistory('agentforce-draft');
        const { doc, page, report } = htmlToCanvas(html);
        if (!doc.artboards.length || !doc.artboards.some((b) => b.boxes.length)) {
            this.aiError =
                'Agentforce returned a document, but nothing in it could be placed on the canvas. Try describing it as a sequence of sections.';
            return;
        }
        if (page) {
        ...
        this.reseedEditor();
        this.aiSummary = res.summary || 'Document drafted.';
        this.aiFindings = this._mapFindings(res.findings);
        this.statusText = 'Drafted ' + (report.boxes || 0) + ' element(s) — nothing is saved until you press Save';
        this.aiConfirmDiscard = false;
    ...
    async connectedCallback() {
        ...
        // Not awaited — the button appears when the answer arrives, and an org with no
        // provider configured simply never shows it.
