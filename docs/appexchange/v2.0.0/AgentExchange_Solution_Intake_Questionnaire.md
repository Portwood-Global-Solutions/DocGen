# AgentExchange Solution Intake Questionnaire — DocGen v2.0.0
...
**Submitted for:** Portwood DocGen Managed v2.0.0 (`04tVx000000ZqBpIAK`)
...
**Portwood DocGen v2.0.0 ships no Agentforce metadata.** The package contains:
...
DocGen v2.0.0 is a **standalone** Managed 2GP package. It does not extend any other managed package. There is no base package dependency.
...
The AppExchange Security Review Dev Org (`dave.2a1209f2e79c@agentforce.com`) has v2.0.0 installed and the post-install configuration completed (DocGen_Admin permset assigned, sample data seeded).
...
| **Authorization**                              | Each invocable opens with an explicit `Schema.sObjectType.<Object>.isAccessible/isCreateable/isUpdateable()` check before any SOQL or DML, per the v2.0 hybrid pattern documented in `DocGen_Solution_Architecture_and_Usage.md`. Standard objects (Account, Contact, etc.) are queried `WITH USER_MODE` so the user's CRUD/FLS is platform-enforced.                                                                                                                                                                                                            |
...
DocGen v2.0.0 ships everything it needs inside the managed package:

- 41 Apex classes + 28 test classes (all namespaced as `portwoodglobal__`)
...
**This questionnaire is non-applicable to Portwood DocGen v2.0.0.** The package contains zero Agentforce elements. No further information is required from this document for the reviewer's security review of the v2.0.0 submission.

For the actual security review of v2.0.0, the relevant documents are:

- `Listing_Describe_Solution.md` / `.pdf` (paste-ready field response)
- `Listing_Platform_Technology.md` / `.pdf` (paste-ready field response)
- `DocGen_Solution_Architecture_and_Usage.md` / `.pdf` (long-form security architecture)
...
All are in `docs/appexchange/v2.0.0/` in the source repository at https://github.com/Portwood-Global-Solutions/DocGen.
