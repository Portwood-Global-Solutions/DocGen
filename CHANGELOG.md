...
## Unreleased — Configurable Email Templates (#193, build pending)

Admin-defined, fully brandable templates for **every** signature/notification email, edited in a rich-text component on a new Command Hub **Email Templates** tab — plus an optional send-time custom message. Replaces the hardcoded HTML that lived in Apex.
...
- **Send-time custom message** — the runner LWC (single-template), the guided/snapshot Apex paths, and the `DocGen: Create Signature Request` Flow action (`Custom Email Message` input) accept an optional message that replaces the `{Message}` token for that send only, never the chrome or button. Packet/bulk and resend paths pass nothing → default templates (bulk send is out of scope for editing but **uses the default templates**).
- **Command Hub "Email Templates" tab** — new `docGenEmailTemplates` LWC: pick an email, edit subject + rich-text body + branding, see a live iframe preview with sample data, send a test email, Save / Reset-to-default. (`DocGenEmailTemplateController`.)
...
- **Tests:** `DocGenEmailTemplateTest` (13 methods: built-in render for all types, escaping, send-time override, stored-record override, brand-color validation, idempotent seeding, controller CRUD/preview/test-send); e2e-06 extended (seed + render assertion).
