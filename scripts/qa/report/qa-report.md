...
**Org** `docgen-verify` · **Run** 2026-07-25T18:25:56.648Z · **Duration** 1871s

## Headline

|                       |              |
| --------------------- | ------------ |
| Checks evaluated      | 1465         |
| Passed                | 1314 (89.7%) |
| Failed                | 151          |
...
| `apex-e2e`       | Apex end-to-end          |     13 |      1 |       0 | 92.9% |
...
- `ui-admin` — closing the modal with unsaved edits warns or preserves them: could not re-open the modal: row "QAUI-0pxbsk-Starter": the menu never offered a visible "Edit" item
...
- ✅ {Now:yyyy-MM-dd HH:mm} formats a timestamp — actual: 2026-07-25 11:40
...
- ✅ HTML→PDF: generation returned bytes — 74847 bytes
- ✅ HTML→PDF: magic bytes are %PDF — leading bytes 255044462D312E34, expected 25504446
- ✅ HTML→PDF: size above 800 bytes — 74847 bytes
...
- ✅ Document_Title_Format\_\_c produces the document title — expected "QAOF004870864-QAOF004870864 Corp-Technology", got "QAOF004870864-QAOF004870864 Corp-Technology"
- ✅ HTML→PDF: result is labelled PDF — outputFormat=PDF templateType=HTML
- ✅ Word→DOCX: generation returned bytes — 1284 bytes
- ✅ Word→DOCX: magic bytes are PK (ZIP) — leading bytes 504B030414000808, expected 504B0304
- ✅ Word→DOCX: size above 400 bytes — 1284 bytes
...
- ✅ Word→DOCX: Document_Title_Format\_\_c applied — expected "QAOF004870864-QAOF004870864 Corp", got "QAOF004870864-QAOF004870864 Corp"
- ✅ Word→PDF: generation returned bytes — 1052 bytes
- ✅ Word→PDF: magic bytes are %PDF — leading bytes 255044462D312E34, expected 25504446
- ✅ Word→PDF: size above 800 bytes — 1052 bytes
...
- ✅ PowerPoint→PPTX: generation returned bytes — 1225 bytes
- ✅ PowerPoint→PPTX: magic bytes are PK (ZIP) — leading bytes 504B030414000808, expected 504B0304
- ✅ PowerPoint→PPTX: size above 400 bytes — 1225 bytes
...
- ✅ PDF AcroForm: merged value is written into the PDF — expected "QAOF004870864 Corp" in the filled PDF bytes; A_MERGED=true
...
- ✅ Deactivated version with an attached file yields a valid PDF or a clean error, never a corrupt one — size=66346 hex=255044462D312E34 threw=false
- ✅ Zero-row child loop: generation returned bytes — 67285 bytes
- ✅ Zero-row child loop: magic bytes are %PDF — leading bytes 255044462D312E34, expected 25504446
- ✅ Zero-row child loop: size above 800 bytes — 67285 bytes
...
- ✅ Header image: generation returned bytes — 90455 bytes
- ✅ Header image: magic bytes are %PDF — leading bytes 255044462D312E34, expected 25504446
- ✅ Header image: size above 800 bytes — 90455 bytes
...
- ✅ Missing image CV: generation returned bytes — 67518 bytes
- ✅ Missing image CV: magic bytes are %PDF — leading bytes 255044462D312E34, expected 25504446
- ✅ Missing image CV: size above 800 bytes — 67518 bytes
...
- ✅ Very large document body: valid PDF or a clean, catchable error — size=505251 hex=255044462D312E34 at body length 1152000
- ✅ Very large document body: PDF is complete, not truncated — %%EOF trailer present in 505251 bytes
- ✅ Very large document body: output size reflects the content — 1152000-char body → 505251 bytes of PDF
...
- ✅ Giant-query PDF: generation returned bytes — 109336 bytes
- ✅ Giant-query PDF: magic bytes are %PDF — leading bytes 255044462D312E34, expected 25504446
- ✅ Giant-query PDF: size above 30000 bytes — 109336 bytes
- ✅ Giant-query PDF is complete, not truncated — %%EOF trailer present in 109336 bytes
- ✅ Giant-query output is named from Document_Title_Format\_\_c — ContentVersion.Title was "QAOF004870864-giant-QAOF004870864 Giant", expected it to start with "QAOF004870864-giant-"
...
- ✅ wizard Back returns to step 1 and keeps what was typed — onStep1=true name="QAUI-0pxbsk-File" (expected "QAUI-0pxbsk-File")
...
- ✅ the wizard creates a template record end to end — a0BO500000OrJujMAF, base object Account
...
- ✅ the starter path creates the template record — a0BO500000OrKHJMA3, HTML/PDF
...
- ✅ search narrows the list to matching rows only — 13 -> 2 rows for "0pxbsk"; every remaining row matches = true
...
- ✅ clicking a column header re-orders the rows both ways — clicked=true; first row "QAUI-0pxbsk-Starter HTML PDF" -> asc "Account HTML PDF Account Act" -> desc "Verify — Landscape Precedenc"
...
- ✅ row action Export downloads a valid .docgen.json bundle — QAUI-0pxbsk-Starter.docgen.json — export version 1, template "QAUI-0pxbsk-Starter"
- ✅ Import Template restores an exported bundle as a new template — "QAUI-0pxbsk-Imported" exists after import
- ✅ row action Clone creates a copy and opens it for editing — created "QAUI-0pxbsk-File (Copy)" (a0BO500000OrHUxMAN); the edit modal opened = true
- ✅ row action Delete removes the template — "QAUI-0pxbsk-File (Copy)" is gone from the org
...
- ✅ edit modal inputs accept real typing — description="edited by ui-admin 0pxbsk" category="QA0pxbsk"
- ✅ the Active toggle flips when clicked — checked true -> false
- ✅ Save as New Version persists the edited fields — stored description="edited by ui-admin 0pxbsk", category="QA0pxbsk" (expected "edited by ui-admin 0pxbsk" / "QA0pxbsk")
- ✅ Save as New Version really creates a new version record — template versions 0 -> 1
- ⊘ closing the modal with unsaved edits warns or preserves them — could not re-open the modal: row "QAUI-0pxbsk-Starter": the menu never offered a visible "Edit" item
...
- ✅ pressing Run creates a bulk job on the server — DocGen_Job\_\_c a03O500003h1FgpIAE status Completed
...
- ✅ the failing job records WHY each record failed — Error_Log\_\_c = 001O5000040jtdsIAA — portwoodglobal.DocGenException: Error retrieving template data: No template file found (active or attached). 001O5000040jtdtIAA — portwoodglobal.DocGenException: Er
- ✅ the Recent Jobs list shows the error count to the user — Recent Jobs did not show "UIQA-ms0q6cyr-err" with "2 errors" — a user would see the run as finished with no indication anything went wrong
