...
- Current shipped version: **v2.0.0** (`04tVx000000ZqBpIAK`) — Security Hardening release for AppExchange resubmission. 4 clickjacking LWC fixes (inline `position: absolute|fixed` → SLDS `slds-is-absolute` class across docGenAdmin/Authenticator/BulkRunner/QueryBuilder/ColumnBuilder); CRUD/FLS hybrid pattern across all admin Apex (`Schema.sObjectType.X.isAccessible/isCreateable/isUpdateable` gate at method entry + SYSTEM_MODE actual op, because USER_MODE strict-FLS strips package-namespaced fields when subscriber admin profiles haven't been granted FLS individually per release); new `DocGenSignatureGuestSecurity.cls` helper documents the guest-token capability model with per-operation field allowlists; verifyDocument now returns ALL signers for multi-signer docs (was LIMIT 1). See `SECURITY_REVIEW_RESPONSE_v2.md` for per-finding mapping.
