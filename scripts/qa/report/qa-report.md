...
**Org** `docgen-verify` · **Run** 2026-07-25T23:36:12.438Z · **Duration** 2321s

## Headline

|                       |              |
| --------------------- | ------------ |
| Checks evaluated      | 1526         |
| Passed                | 1368 (89.6%) |
| Failed                | 158          |
| Skipped (not counted) | 10           |
| Blockers              | 6            |
| Major                 | 8            |
| Minor                 | 144          |
...
| `apex-e2e`       | Apex end-to-end          |     12 |      4 |       0 |   75% |
| `apex-unit`      | Apex unit                |     45 |     15 |       0 |   75% |
...
| `ui-runner`      | End-user UI              |     75 |      5 |       0 | 93.8% |
...
| ----------- | ---------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **blocker** | `apex-e2e`       | Signatures: e2e-06-signatures.apex                                                                             | 1 of 25 assertions failed                                                                                                                                                                                                                                                                                      |
| **blocker** | `apex-e2e`       | Signature redemption: e2e-06c-signing.apex                                                                     | 1 of 1 assertions failed                                                                                                                                                                                                                                                                                       |
| **blocker** | `apex-e2e`       | PIN gate + decline: e2e-06d-pin-and-decline.apex                                                               | 1 of 1 assertions failed                                                                                                                                                                                                                                                                                       |
| **blocker** | `apex-unit`      | the Apex test run passes                                                                                       | 2 failing: portwoodglobal.DocGenSignatureTests.testSendPin_expiredToken, portwoodglobal.DocGenSignatureTests.testSendPin_success                                                                                                                                                                               |
| **blocker** | `apex-unit`      | portwoodglobal.DocGenSignatureTests.testSendPin_expiredToken passes                                            | System.AssertException: Assertion Failed: Expected: true, Actual: false                                                                                                                                                                                                                                        |
| **blocker** | `apex-unit`      | portwoodglobal.DocGenSignatureTests.testSendPin_success passes                                                 | System.AssertException: Assertion Failed: PIN send should succeed: Expected: true, Actual: false                                                                                                                                                                                                               |
...
| **major**   | `ui-runner`      | a user without the DocGen permission set gets a clear message, not a broken UI                                 | the component rendered nothing at all — an unentitled user sees an empty panel with no explanation of why                                                                                                                                                                                                      |
| **major**   | `ui-runner`      | building a Document Packet produces a file                                                                     | two templates were moved into the packet (Account, Account) and the generate button pressed, but no download arrived within 300s                                                                                                                                                                               |
...
- `ui-admin` — closing the modal with unsaved edits warns or preserves them: could not re-open the modal: row "QAUI-10zero-Starter": the menu never offered a visible "Edit" item
...
- ❌ Signatures: e2e-06-signatures.apex — 1 of 25 assertions failed
- ❌ Signature lifecycle: e2e-06b-signature-lifecycle.apex — 1 of 1 assertions failed
- ❌ Signature redemption: e2e-06c-signing.apex — 1 of 1 assertions failed
- ❌ PIN gate + decline: e2e-06d-pin-and-decline.apex — 1 of 1 assertions failed
...
- ❌ the Apex test run passes — 2 failing: portwoodglobal.DocGenSignatureTests.testSendPin_expiredToken, portwoodglobal.DocGenSignatureTests.testSendPin_success
- ❌ portwoodglobal.DocGenSignatureTests.testSendPin_expiredToken passes — System.AssertException: Assertion Failed: Expected: true, Actual: false
- ❌ portwoodglobal.DocGenSignatureTests.testSendPin_success passes — System.AssertException: Assertion Failed: PIN send should succeed: Expected: true, Actual: false
...
- ✅ DocGenSignatureController meets the 75% packaging bar — 85% (1227/1449 lines)
...
- ✅ org-wide coverage is at or above 75% — 78% (23102/29461 lines) — a 2GP build fails below 75%
...
- ✅ {Now:yyyy-MM-dd HH:mm} formats a timestamp — actual: 2026-07-25 16:49
...
- ✅ HTML→PDF: generation returned bytes — 75130 bytes
- ✅ HTML→PDF: magic bytes are %PDF — leading bytes 255044462D312E34, expected 25504446
- ✅ HTML→PDF: size above 800 bytes — 75130 bytes
...
- ✅ Document_Title_Format\_\_c produces the document title — expected "QAOF023445349-QAOF023445349 Corp-Technology", got "QAOF023445349-QAOF023445349 Corp-Technology"
...
- ✅ Word→DOCX: Document_Title_Format\_\_c applied — expected "QAOF023445349-QAOF023445349 Corp", got "QAOF023445349-QAOF023445349 Corp"
...
- ✅ PDF AcroForm: merged value is written into the PDF — expected "QAOF023445349 Corp" in the filled PDF bytes; A_MERGED=true
...
- ✅ Deactivated version with an attached file yields a valid PDF or a clean error, never a corrupt one — size=66690 hex=255044462D312E34 threw=false
- ✅ Zero-row child loop: generation returned bytes — 67854 bytes
- ✅ Zero-row child loop: magic bytes are %PDF — leading bytes 255044462D312E34, expected 25504446
- ✅ Zero-row child loop: size above 800 bytes — 67854 bytes
...
- ✅ Header image: generation returned bytes — 90955 bytes
- ✅ Header image: magic bytes are %PDF — leading bytes 255044462D312E34, expected 25504446
- ✅ Header image: size above 800 bytes — 90955 bytes
...
- ✅ Missing image CV: generation returned bytes — 68074 bytes
- ✅ Missing image CV: magic bytes are %PDF — leading bytes 255044462D312E34, expected 25504446
- ✅ Missing image CV: size above 800 bytes — 68074 bytes
...
- ✅ Very large document body: valid PDF or a clean, catchable error — size=505688 hex=255044462D312E34 at body length 1152000
- ✅ Very large document body: PDF is complete, not truncated — %%EOF trailer present in 505688 bytes
- ✅ Very large document body: output size reflects the content — 1152000-char body → 505688 bytes of PDF
...
- ✅ Giant-query output is named from Document_Title_Format\_\_c — ContentVersion.Title was "QAOF023445349-giant-QAOF023445349 Giant", expected it to start with "QAOF023445349-giant-"
...
- ✅ Designer tab offers templates to open when none is loaded — {"list":true,"count":8,"first":"PDFQA Giant ChromeAccountDesign →"}
- ✅ template list stays bounded regardless of org size — 8 rendered · Showing 8 of 12 · 12 total
...
- ✅ wizard Back returns to step 1 and keeps what was typed — onStep1=true name="QAUI-10zero-File" (expected "QAUI-10zero-File")
...
- ✅ the wizard creates a template record end to end — a0BO500000OrfbtMAB, base object Account
...
- ✅ the starter path creates the template record — a0BO500000OrfdVMAR, HTML/PDF
...
- ✅ the template list renders rows — 14 rows; count label "14 templates"
- ✅ search narrows the list to matching rows only — 14 -> 2 rows for "10zero"; every remaining row matches = true
- ✅ the row-count label reports the filtered subset — label reads "2 of 14 templates"
- ✅ a search with no matches empties the list instead of ignoring the query — 0 rows survived a nonsense query
- ✅ clearing the search restores the full list — 14 rows (expected 14)
- ✅ clicking a column header re-orders the rows both ways — clicked=true; first row "QAUI-10zero-Starter HTML PDF" -> asc "Account HTML PDF Account Act" -> desc "Verify — Landscape Precedenc"
- ✅ Refresh reloads the list without emptying it — hit-test=ok; 14 rows after refresh (expected 14)
...
- ✅ row action Export downloads a valid .docgen.json bundle — QAUI-10zero-Starter.docgen.json — export version 1, template "QAUI-10zero-Starter"
- ✅ Import Template restores an exported bundle as a new template — "QAUI-10zero-Imported" exists after import
- ✅ row action Clone creates a copy and opens it for editing — created "QAUI-10zero-File (Copy)" (a0BO500000OrfnBMAR); the edit modal opened = true
- ✅ row action Delete removes the template — "QAUI-10zero-File (Copy)" is gone from the org
...
- ✅ edit modal inputs accept real typing — description="edited by ui-admin 10zero" category="QA10zero"
- ✅ the Active toggle flips when clicked — checked true -> false
- ✅ Save as New Version persists the edited fields — stored description="edited by ui-admin 10zero", category="QA10zero" (expected "edited by ui-admin 10zero" / "QA10zero")
- ✅ Save as New Version really creates a new version record — template versions 0 -> 1
- ⊘ closing the modal with unsaved edits warns or preserves them — could not re-open the modal: row "QAUI-10zero-Starter": the menu never offered a visible "Edit" item
...
- ✅ the generated document's title resolves merge tokens against the record — Document_Title_Format\_\_c = "UIQA-ms118ez6 {Name}" → title was "UIQA-ms118ez6 UIQA Alpha", expected "UIQA-ms118ez6 UIQA Alpha"
...
- ✅ docGenRunner renders on a record page — rendered 554 chars, 2 picker(s), 1 primary button(s)
...
- ✅ the record-page template picker lists a template the user can actually run — picker showed: Choose a template... / Account / [Test] E2E Test Template / ffff / PDFQA Giant Chrome / playa / test / Test2 / test3 / test345 / test99 / [UIQA] UIQA Good PDF / [UIQA] UIQA Locked Forma
...
- ✅ pressing Create Document in Save to Record mode puts the document ON the record — ContentDocument "UIQA-ms118ez6 UIQA Alpha" linked to 001O5000040oNAqIAM
...
- ✅ pressing Create Document in Download mode downloads the document to the browser — the browser received "UIQA-ms118ez6 UIQA Alpha.pdf"
...
- ✅ the packet chooser offers the record's PDF templates — chooser offered 17 template(s): Account / E2E Test Template / ffff / PDFQA Giant Chrome / playa / test
...
- ✅ sending a signature request tells the user it worked — component text: Signature Links Generated! Provide each signer with their unique secure link: UIQA-ms118ez6 Signer Signer https://<CONFIGURE_SITE_URL_IN_SETUP>/apex/portwoodglobal\_\_DocGenSignaturePdf?
- ✅ sending writes a DocGen_Signature_Request\_\_c tied to this record and template — request a08O5000013zA0DIAU: record=001O5000040oNAqIAM (expected 001O5000040oNAqIAM), status=Sent, order=Parallel, sourceDoc=068O500000MICm1IAH
- ✅ sending writes exactly one DocGen_Signer\_\_c carrying what was typed — 1 signer row(s) (expected 1); first = name "UIQA-ms118ez6 Signer" (typed "UIQA-ms118ez6 Signer"), email "uiqa-ms118ez6@example.com" (typed "uiqa-ms118ez6@example.com"), role "Signer" (typed "Signer"),
...
- ✅ focusing the template box lists the available templates — 18 options offered
...
- ✅ pressing Run creates a bulk job on the server — DocGen_Job\_\_c a03O500003h71bNIAQ status Completed
...
- ✅ the failing job records WHY each record failed — Error_Log\_\_c = 001O5000040oNAqIAM — portwoodglobal.DocGenException: Error retrieving template data: No template file found (active or attached). 001O5000040oNArIAM — portwoodglobal.DocGenException: Er
- ✅ the Recent Jobs list shows the error count to the user — Recent Jobs did not show "UIQA-ms118ez6-err" with "2 errors" — a user would see the run as finished with no indication anything went wrong
...
- ❌ a user without the DocGen permission set gets a clear message, not a broken UI — the component rendered nothing at all — an unentitled user sees an empty panel with no explanation of why
- ❌ building a Document Packet produces a file — two templates were moved into the packet (Account, Account) and the generate button pressed, but no download arrived within 300s
...
- ✅ the giant template generates — 442611 bytes in 5008ms (CPU 4780ms of 10000)
- ✅ the giant render leaves CPU headroom — 4780ms of the 10000ms synchronous limit (48%) at 2100 rows
